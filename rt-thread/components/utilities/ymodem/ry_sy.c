...
rt_device_t ry_dev;
const char *ry_file_path;
rt_thread_t finsh_tid;

void ry_thread_entry(void *parameter)
{
    rt_err_t res;
    res = rym_download_file(ry_dev,ry_file_path);
    rt_thread_resume(finsh_tid);
    return ;
}
...
static rt_err_t ry(uint8_t argc, char **argv)
{
    ...
    if (argc > 2)
        ry_dev = rt_device_find(argv[2]);
    else
        ry_dev = rt_console_get_device();
    if (!ry_dev)
    {
    ...
    tid = rt_thread_create("rymodem",
                           ry_thread_entry, RT_NULL,
                           4096, 9, 10);
    if (tid != NULL)
    {
        finsh_tid = rt_thread_self();
        res = rt_thread_suspend(finsh_tid);
        if(res == RT_EOK)
