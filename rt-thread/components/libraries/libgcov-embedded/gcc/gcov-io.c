...
gcov_close (void)
{
  ...
  if (gcov_var.file)
    ...
    gcov_write_block (gcov_var.offset);
      ...
      gcov_var.file = 0;
#else
      long fd =  semihosting_file_open(gcov_var.filename, FOPEN_MODE_WB);
      size_t length = gcov_var.gcda_buf_pos - gcov_var.gcda_buf;
      semihosting_file_write(fd, gcov_var.gcda_buf, &length);
