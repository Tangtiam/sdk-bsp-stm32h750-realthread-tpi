...
{
    rt_device_t device;
    rt_uint32_t flags = (rt_uint32_t)args;
    rt_uint32_t mask  = O_NONBLOCK | O_APPEND;
...
static int _fops_read(struct dfs_file *fd, void *buf, size_t count)
{
    int size = 0;
    rt_device_t device;
...
static int _fops_write(struct dfs_file *fd, const void *buf, size_t count)
{
    rt_device_t device;
                        ...
                        rt_size_t size)
{
    ...
    rt_size_t index = 0, lf = 0;
    rt_size_t i;

    if (vc->is_writing)
        return 0;

    vc->is_writing = RT_TRUE;
    w_len = 0;
    bstr = vc->cache;
